...
describe("Store endpoints", function () {
  ...
  it("Fetch store with UserId", async function () {
    const response = await request(app).get(
      "/store/user/64ed5f7942e9c21d8fe5e699"
    );
  ...
  it("Create store", async function () {
    const response = await request(app).post("/store").send({
      name: "Halo Store",
      location: "Langit biru terang benderang",
      UserId: "64ed60a5aba1d9191cb25b5b",
    });

    expect(response.status).toEqual(201);
    expect(response.body).toHaveProperty("message", "Store created");
  });

  it("Edit store", async function () {
    const response = await request(app).put("/store/1").send({
      name: "Takut",
      location: "Gataww",
      profileImg: "fotofotokeren.jpg",
    });
